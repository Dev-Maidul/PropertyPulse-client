...
const PropertyBought = () => {
  ...
  return (
    <div className="w-full mx-auto px-2 sm:px-4 py-8">
      <h1 className="text-3xl font-bold text-property-secondary mb-8 text-center">My Property Offers</h1>
                ...
                <td className="p-3">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold
                    ${offer.status === "pending" ? "bg-yellow-100 text-yellow-700" :
                      offer.status === "accepted" ? "bg-blue-100 text-blue-700" :
                      offer.status === "bought" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-700"}
                  `}>
                ...
                <td className="p-3">
                  ...
                  {offer.status === "bought" && offer.transactionId && (
                    <div className="text-green-600 font-semibold text-xs">
                      Paid<br />
                      <span className="font-mono">Txn: {offer.transactionId}</span>
                    </div>
                  )}
                  {offer.status === "pending" && (
                    <span className="text-gray-400 text-xs">Waiting for agent</span>
                  )}
                </td>
              </tr>
            ))}
